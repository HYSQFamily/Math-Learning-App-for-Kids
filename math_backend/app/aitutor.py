...
from .deepseek_client import DeepSeekClient, DeepSeekRequest
import httpx
...
@router.post("/ask")
async def ask_tutor(req: TutorRequest, service: str = Query("openai", description="AI service to use (openai or deepseek)")):
    try:
        if service == "deepseek":
            try:
                client = DeepSeekClient()
                answer = await client.ask(DeepSeekRequest(user_id=req.user_id, question=req.question))
                print(f"DEBUG: Received answer from Deepseek => {answer}")
                return {"answer": answer}
            except httpx.HTTPError as e:
                print(f"ERROR: Deepseek API error => {str(e)}")
                try:
                    # Fallback to OpenAI
                    print("DEBUG: Falling back to OpenAI")
                    return await ask_openai(req)
...
async def ask_openai(req: TutorRequest):
